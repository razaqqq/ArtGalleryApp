...
import AppleAuthbtnComponent from './AppleAuthBtnComponent';
...
describe('AppleAuthButton Test', () => {
    afterEach(() => {
		jest.resetAllMocks();
	});

    it('should iniate OAuth flow when AppleAuthBtn button is clicked', async () => {
		...
		const { getByTestId } = render(<AppleAuthbtnComponent />);

		const button = getByTestId('AuthButtonTestID');
    ...
    it('should sign in successfully with Apple OAuth', async () => {
		const startOAuthFlowMock = jest.fn().mockResolvedValue({
			createdSessionId: 'test-session-id',
			setActive: jest.fn(),
		});
