...
import ProfilePreviewComponent from "./ProfilePreview.component";
...
describe('Profile Preview', () => {
	it('should display default values for missing user information', async () => {
		...
		const { queryByText } = render(<ProflePreviewComponent user={user} />);
	...
	it('should display a default profile image if the user has no profile image URL', async () => {
		...
		const { findByTestId } = render(<ProfilePreviewComponent user={user} />);
	...
	it('should not render anything if fonts fails to load', () => {
		...
		const { queryByText } = render(<ProfilePreviewComponent user={user} />);
